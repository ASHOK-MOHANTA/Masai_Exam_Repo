...
function generateInsights(data){
    ...
    let avgPopulation = Math.round(data.reduce((sum,country)=> sum + country.population, 0) / countryCount);

    const top5ByPopulation = data.slice()
                                 .sort((a,b)=> b.population - a.population)
                                 .slice(0,5)
                                 .map(country=> country.name);
                                 
     const populationByContinaent = data.reduce((acc,country)=>{
        acc[country.region] = (acc[country.region]) + country.population;
        return acc;
     },);

     const densestCountry = data.map(country =>({
        name: country.name,
        density: Math.round(country.population / country.area)
     }))
        .sort((a,b)=> b.density - a.density)
        .slice(0,3);
    const lowAreaCountries = data.sort((a,b)=> a.area - b.area)
        .slice(0,5)
        .map(country=> country.name);   
        return {
        countryCount,
...
console.log(generateInsights(countries));
